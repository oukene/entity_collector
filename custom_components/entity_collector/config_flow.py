            ...
            if not errors:
                ...
                for key in all_entities_by_id:
                    if all_entities_by_id[key]["entity_id"] not in user_input[CONF_ENTITIES]:
                        ...
                        _LOGGER.debug("append entity : %s, %s", key[0], key[1])
    ...
    async def async_step_entity(self, user_input: Optional[Dict[str, Any]] = None):
        ...
        if user_input is not None:
            ...
            if not errors:
                ...
                _LOGGER.debug("entity : %s", user_input[CONF_ORIGIN_ENTITY])
                _LOGGER.debug("name : %s", user_input[CONF_NAME])
                name = user_input.get(CONF_NAME)
                if name == "" or name == None:
                    _LOGGER.debug("set name")
                    t = user_input[CONF_ORIGIN_ENTITY]
                    name = t.replace(t.split(".")[0] + ".", "") 
